...
def predict_patch(args, model, augment='default'):
  result_df = None
  model = load_model(args) if model is None else model
  IMAGE_DIR = "/data/kaggle_dataset/PUBLICHPA"
  im = "672_E2_1"
  image = read_rgb(os.path.join(IMAGE_DIR,"images","train",im))
  data = image[:, 60:188, 120:248]
  with torch.no_grad():
      outputs = model(data)
      logits = outputs['logits']
      print(outputs, logits)
...
def main(args):
  ...
  result_df = predict_cell_result(args, test_loader, model)
  #result_df = predict_patch(args, model)
  end_time = timer()
